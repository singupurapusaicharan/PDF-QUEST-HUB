  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Welcome to PDF Quest Hub.",
      });
      navigate('/dashboard');
    } catch (error) {
      toast({
        ...
        <div className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl border border-gray-200/50 dark:border-gray-800/50 rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-12 bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-white border-0"
            >
              {loading ? (
          ...
          <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-6">
            ...
            <Link to="/signin" className="text-teal-600 dark:text-teal-400 hover:underline font-medium">
              Sign in
            </Link>
          </p>
        </div>
